...
def is_proposition(s):
    ...
    if parsed[1] == '<': valid_relation = True
    if parsed[1] == 'A<': valid_relation = True
    if parsed[1] == 'a<': valid_relation = True
    if parsed[1] == 'r<': valid_relation = True
    if parsed[1] == 't<': valid_relation = True
    if parsed[1] == 'e<': valid_relation = True
    if parsed[1] == 'osep': valid_relation = True
    if parsed[1] == 'asep': valid_relation = True
    if parsed[1] == 'rsep': valid_relation = True
    if parsed[1] == 'tsep': valid_relation = True
    if parsed[1] == 'sep': valid_relation = True
    if parsed[1] == '=': valid_relation = True
    if parsed[1] == '!<': valid_relation = True
    if parsed[1] == '!A<': valid_relation = True
    if parsed[1] == '!a<': valid_relation = True
    ...
    if parsed[1] == 't<?': valid_relation = True
    if parsed[1] == 'e<?': valid_relation = True
    if parsed[1] == 'z=': valid_relation = True
    if parsed[1] == 'p=': valid_relation = True
...
def get_keywords(d):
    ...
    parsed = d.split(':')
    if len(parsed) < 3: return keywords

    keywords = parsed[2].strip()
    keywords = keywords.split(',')
    for keyword in keywords: keyword = keyword.strip()
...
def parse(classes_file, operators_file):
    ...
    zoomodules.ops.no_operator_declarations(classes)
    classes = op_classes(classes_file, classes, operators)
    classes = zoomodules.ops.generate_co(classes)
